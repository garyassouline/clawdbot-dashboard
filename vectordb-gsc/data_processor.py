...
def process_discover_pages(raw_data):
    """Aggregate Discover data into page-level documents (no query dimension)."""
    rows = raw_data.get("rows", [])
    if not rows:
        console.print("  [yellow]No Discover rows to process.[/yellow]")
        return []
    ...
    page_agg = defaultdict(lambda: {
        ...
        "countries": set(), "devices": set(),
        "monthly_clicks": defaultdict(int),
        "monthly_impressions": defaultdict(int),
        "dates": set(),
    })
    ...
    for row in rows:
        ...
        page, date_str, device, country = keys[:4]
        clicks = row.get("clicks", 0)
        impressions = row.get("impressions", 0)
        ctr = row.get("ctr", 0)
        month_key = date_str[:7]
        ...
        entry["devices"].add(device)
        entry["monthly_clicks"][month_key] += clicks
        entry["monthly_impressions"][month_key] += impressions
        entry["dates"].add(date_str)
    ...
    for page, data in page_agg.items():
        ...
        # Extract page slug for readability
        slug = page.rstrip("/").split("/")[-1] if "/" in page else page

        text = (
            f"Discover Page: {page} | Slug: {slug} | "
            f"Clicks: {data['clicks']:,} | Impressions: {data['impressions']:,} | "
            f"CTR: {avg_ctr}% | Trend: {trend} | "
            f"Devices: {','.join(sorted(data['devices']))} | "
            f"Countries: {','.join(sorted(list(data['countries'])[:5]))}"
        )

        doc_id = f"disc_{hash(page) & 0xFFFFFFFF:08x}"
        metadata = {
            "page": page,
            "clicks": data["clicks"],
            "impressions": data["impressions"],
            "ctr": avg_ctr,
            "trend": trend,
            "countries": ",".join(sorted(list(data["countries"])[:10])),
            "devices": ",".join(sorted(data["devices"])),
            "first_date": min(data["dates"]),
            "last_date": max(data["dates"]),
            "data_points": data["count"],
        }
        discover_docs.append({"id": doc_id, "text": text, "metadata": metadata})
    ...
    summary = Table(
        ...
        title="[bold]Discover Processing Summary[/bold]",
        title_style="bold white",
        padding=(0, 2),
    )
    summary.add_column("Metric", style="white")
    summary.add_column("Count", style="cyan", justify="right")
    summary.add_row("Raw rows", f"{len(rows):,}")
    summary.add_row("Unique pages", f"{len(discover_docs):,}")
    summary.add_row("", "")
    for t in ["rising", "stable", "declining", "new"]:
        style = {"rising": "green", "stable": "white", "declining": "red", "new": "blue"}.get(t, "white")
        summary.add_row(f"  [{style}]{t.capitalize()}[/{style}] pages", f"[{style}]{trends.get(t, 0):,}[/{style}]")
...
def process_discover_data(raw_data_path=None):
    if raw_data_path is None:
        raw_data_path = os.path.join(config.RAW_DATA_DIR, "gsc_discover_data.json")
    raw_data = load_raw_data(raw_data_path)
    if raw_data is None:
        return None
    return process_discover_pages(raw_data)
