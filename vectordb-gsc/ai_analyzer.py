...
DISCOVER_PROMPTS = {
    "discover_overview": {
        "title": "Discover Performance Overview",
        "description": "Top pages, CTR, and traffic trends in Google Discover",
        "query_text": "top performing pages clicks impressions discover feed",
        "system": (
            f"You are an expert SEO analyst specializing in Google Discover. {YEAR_CONTEXT} {NO_SCHEMA} "
            "Analyze the Google Discover performance data provided. "
            "Google Discover surfaces content to users based on interests, not search queries. "
            "Give a comprehensive overview including:\n"
            "1. Top performing pages by clicks in Discover\n"
            "2. Overall traffic trends (rising, declining, stable)\n"
            "3. CTR patterns — what makes content perform well in the feed\n"
            "4. Content types/topics that get the most Discover visibility\n"
            "5. Device breakdown (mobile dominates Discover)\n"
            "Use specific numbers from the data. Be concise but thorough."
        ),
        "n_results": 50,
    },
    "discover_winners": {
        "title": "Discover Winners & Patterns",
        "description": "What content types perform best in the Discover feed",
        "query_text": "high clicks high CTR best performing discover content",
        "system": (
            f"You are an expert SEO analyst specializing in Google Discover. {YEAR_CONTEXT} {NO_SCHEMA} "
            "Analyze the Google Discover data to find winning patterns:\n"
            "1. Which pages get the most Discover clicks? What do they have in common?\n"
            "2. What CTR ranges are typical? Which pages beat the average?\n"
            "3. Content format patterns: listicles vs guides vs news vs recipes\n"
            "4. Freshness factor: do newer pages perform better?\n"
            "5. Topic clusters that consistently perform well\n"
            "6. Actionable recommendations: what content to create more of\n"
            "Focus on extracting replicable patterns from the data."
        ),
        "n_results": 50,
    },
    "discover_declining": {
        "title": "Discover Declining Pages",
        "description": "Pages losing Discover visibility over time",
        "query_text": "declining falling dropping discover traffic loss",
        "system": (
            f"You are an expert SEO analyst specializing in Google Discover. {YEAR_CONTEXT} {NO_SCHEMA} "
            "Analyze pages losing Google Discover traffic:\n"
            "1. Which pages show declining Discover performance?\n"
            "2. Is the decline seasonal, topic-related, or systemic?\n"
            "3. Content freshness issues — is old content losing Discover eligibility?\n"
            "4. Are there algorithmic patterns (e.g., Discover favoring different content types now)?\n"
            "5. Recovery strategies specific to Discover\n"
            "Remember: Discover is highly volatile — distinguish normal fluctuation from real decline."
        ),
        "n_results": 50,
        "where": {"trend": "declining"},
    },
...
def _gather_discover_context(analysis_key, custom_query=None, large_context=False):
    """Retrieve relevant Discover documents from ChromaDB."""
    client = vector_store.get_chroma_client()
    discover_col = vector_store.get_or_create_collection(client, config.DISCOVER_COLLECTION)
    ...
    if custom_query:
        ...
        prompt_config = DISCOVER_PROMPTS[analysis_key]
        query_text = prompt_config["query_text"]
        n_results = GROK_N_RESULTS if large_context else prompt_config.get("n_results", 30)
        where = prompt_config.get("where")

    results = vector_store.query_collection(
        discover_col, query_text, n_results=n_results, where=where
    )

    d_count = len(results["documents"][0]) if results and results["documents"] else 0
    console.print(f"  [dim]Retrieved[/dim] [magenta]{d_count}[/magenta] [dim]Discover page docs from vector DB[/dim]\n")

    context_parts = ["## Google Discover Page Performance Data\n"]
    ...
    stats = vector_store.get_collection_stats()
    context_parts.append(f"\n## Database Stats")
    context_parts.append(f"Total Discover pages in database: {stats.get(config.DISCOVER_COLLECTION, 0):,}")

    return "\n".join(context_parts)


def run_discover_analysis(analysis_key, provider="gemini"):
    """Run a Discover-specific analysis."""
    prompt_config = DISCOVER_PROMPTS.get(analysis_key)
    if not prompt_config:
        console.print(f"  [red]Unknown Discover analysis: {analysis_key}[/red]")
        return None
    ...
    console.print()
    context = _gather_discover_context(analysis_key, large_context=large_context)

    with Live(
        Panel(f"  Generating Discover analysis with {provider_label}...", border_style="dim", padding=(1, 2)),
        console=console, transient=True,
    ):
        if provider == "grok":
            result = analyze_with_grok(system_prompt, context)
        elif provider == "claude":
            result = analyze_with_claude(system_prompt, context)
        else:
            result = analyze_with_gemini(system_prompt, context)
