...
def _parse_flag(flag, default=None):
...
def print_help():
    ...
    table.add_row("[bold]Search[/bold]", "")
    table.add_row("extract", "Pull last 16 months of GSC Search data via API")
    table.add_row("process", "Process raw Search data and store embeddings")
    table.add_row("refresh", "Extract + process Search in one step")
    table.add_row("analyze", "Interactive AI analysis session (Search)")
    table.add_row('ask "question"', "One-shot question (default: Gemini Flash)")
    table.add_row("", "")
    table.add_row("[bold]Discover[/bold]", "")
    table.add_row("extract-discover", "Pull last 16 months of Discover data via API")
    table.add_row("process-discover", "Process Discover data and store embeddings")
    table.add_row("refresh-discover", "Extract + process Discover in one step")
    table.add_row("", "")
    ...
    table.add_row("export", "Export all analyses as JSON for Marmiton HQ dashboard")
    table.add_row("stats", "Show vector database statistics")
    table.add_row("help", "Show this help message")
    table.add_row("", "")
    table.add_row("[dim]Flags[/dim]", "[dim]--grok  --claude  --output <path>[/dim]")
...
def cmd_extract_discover():
    print_banner()
    gsc_extractor.extract_discover_data()
...
def cmd_process_discover():
    print_banner()
    console.print(Panel("[bold]Step 1/2[/bold]  Processing Discover data", border_style="magenta"))
    discover_docs = data_processor.process_discover_data()
    if discover_docs is None:
        return
    console.print()
    console.print(Panel("[bold]Step 2/2[/bold]  Embedding & storing in ChromaDB", border_style="magenta"))
    vector_store.store_discover(discover_docs)
    console.print()
    _print_stats_inline()
...
def cmd_refresh_discover():
    print_banner()
    console.print(Panel("[bold]Step 1/3[/bold]  Extracting Discover data", border_style="magenta"))
    filepath = gsc_extractor.extract_discover_data()
    if filepath is None:
        return
    console.print()
    console.print(Panel("[bold]Step 2/3[/bold]  Processing Discover data", border_style="magenta"))
    discover_docs = data_processor.process_discover_data(filepath)
    if discover_docs is None:
        return
    console.print()
    console.print(Panel("[bold]Step 3/3[/bold]  Embedding & storing in ChromaDB", border_style="magenta"))
    vector_store.store_discover(discover_docs)
    console.print()
    _print_stats_inline()
...
def cmd_export(output_dir, provider="gemini"):
    print_banner()
    dashboard_export.export_to_dashboard(output_dir, provider=provider)
...
def _print_stats_inline():
    ...
    table.add_row("Pages (Search)", f"{pg:,}", config.EMBEDDING_MODEL)
    table.add_row("Pages (Discover)", f"{disc:,}", config.EMBEDDING_MODEL)
    table.add_row("", "", "")
    table.add_row("[bold]Total[/bold]", f"[bold]{qp + pg + disc:,}[/bold]", f"{config.EMBEDDING_DIMENSIONS}d vectors")
...
def main():
    ...
    simple_commands = {
        "extract": cmd_extract,
        "extract-discover": cmd_extract_discover,
        "process": cmd_process,
        "process-discover": cmd_process_discover,
        "refresh": cmd_refresh,
        "refresh-discover": cmd_refresh_discover,
        "analyze": cmd_analyze,
        "stats": cmd_stats,
        "help": print_help,
    }
    ...
    if command == "export":
        output = _parse_flag("--output", default="../clawdbot-dashboard/ai_data/")
        cmd_export(output, _parse_provider())
