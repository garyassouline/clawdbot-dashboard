...
def store_discover(discover_docs):
    """Store Discover page documents in their own collection."""
    client = get_chroma_client()
    if discover_docs:
        col = get_or_create_collection(client, config.DISCOVER_COLLECTION)
        console.print(f"  [dim]Collection:[/dim] [magenta]{config.DISCOVER_COLLECTION}[/magenta]  ({len(discover_docs):,} documents)")
        upsert_documents(col, discover_docs)
    console.print()
    console.print("  [bold green]Discover documents stored successfully.[/bold green]")


def get_collection_stats():
    client = get_chroma_client()
    stats = {}
    for name in [config.QUERIES_COLLECTION, config.PAGES_COLLECTION, config.DISCOVER_COLLECTION]:
        try:
